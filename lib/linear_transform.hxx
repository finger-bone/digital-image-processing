...
using interplate_function = std::function<BmpImage::BmpPixel(
    BmpImage::BmpPixel, BmpImage::BmpPixel, BmpImage::BmpPixel,
    BmpImage::BmpPixel, double, double)>;

double f_part(double x) { return x - floor(x); }

BmpImage::BmpPixel bilinear_interpolate(BmpImage::BmpPixel top_left,
                                        BmpImage::BmpPixel top_right,
                                        BmpImage::BmpPixel bottom_left,
                                        BmpImage::BmpPixel bottom_right,
                                        double x_from_top_left,
                                        double y_from_top_left) {
  auto map_one_value = [&](uint8_t top_left, uint8_t top_right,
  ...
  return BmpImage::BmpPixel{
      .red = map_one_value(top_left.red, top_right.red, bottom_left.red,
                           bottom_right.red, x_from_top_left, y_from_top_left),
      .green =
          map_one_value(top_left.green, top_right.green, bottom_left.green,
                        bottom_right.green, x_from_top_left, y_from_top_left),
      .blue =
          map_one_value(top_left.blue, top_right.blue, bottom_left.blue,
                        bottom_right.blue, x_from_top_left, y_from_top_left),
      .alpha =
          map_one_value(top_left.alpha, top_right.alpha, bottom_left.alpha,
                        bottom_right.alpha, x_from_top_left, y_from_top_left),
  };
}

BmpImage::BmpImage
linear_transform(BmpImage::BmpImage image, Linalg::Matrix<double> matrix,
  ...
  auto pixels = image.image.data.interpret(image.image.size.height,
                                           image.image.size.width);
  ...
  result_image.image.data.foreach ([&](BmpImage::BmpPixel &pixel, size_t idx) {
    ...
    pixel = bilinear_interpolate(
        get_pixel(floor(std::get<0>(pre_transformed_point_divided)),
                  floor(std::get<1>(pre_transformed_point_divided))),
        get_pixel(ceil(std::get<0>(pre_transformed_point_divided)),
                  floor(std::get<1>(pre_transformed_point_divided))),
        get_pixel(floor(std::get<0>(pre_transformed_point_divided)),
                  ceil(std::get<1>(pre_transformed_point_divided))),
        get_pixel(ceil(std::get<0>(pre_transformed_point_divided)),
                  ceil(std::get<1>(pre_transformed_point_divided))),
