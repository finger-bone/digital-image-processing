...
RealMatrix hough_linear_transform(const RealMatrix& matrix, HoughLineParam& param, bool rect_mode = false, double rect_tolerant = 0.05) {
    auto& [theta_steps, rho_steps, rho_max] = param;

    if (rho_max == -1) { // 自动计算 rho_max
        rho_max = std::sqrt(matrix.size() * matrix.size() + matrix[0].size() * matrix[0].size());
    }
        ...
        for (int x = 0; x < matrix[0].size(); ++x) {
            if (matrix[y][x] > 1e-5) {
                for (int t_i = 0; t_i < theta_steps; ++t_i) {
                    ...
                    int r_i = static_cast<int>((rho + rho_max) * rho_steps / (2 * rho_max));
...
std::vector<std::tuple<double, double>> get_lines(const RealMatrix& matrix, HoughLineParam& param, double threshold = -1, double auto_ratio=0.5) {
    auto& [theta_steps, rho_steps, rho_max] = param;
            ...
            if (matrix[t_i][r_i] > threshold) {
                ...
                for (int dt = -1; dt <= 1; ++dt) {
                    for (int dr = -1; dr <= 1; ++dr) {
                        ...
                        int nr = r_i + dr;
                        if (nr >= 0 && nr < rho_steps && matrix[nt][nr] > matrix[t_i][r_i]) {
                            is_peak = false;
                            break;
                        }
                    }
...
double calculate_distance(int t1, int r1, int t2, int r2, int theta_steps, int rho_steps) {
    return sqrt(
        (t1 - t2) * (t1 - t2) + (r1 - r2) * (r1 - r2)
    );
}

// BFS to find nearby points within spread distance
std::vector<std::tuple<int, int>> bfs(const RealMatrix& matrix, int start_t, int start_r, double threshold, int spread, int theta_steps, int rho_steps) {
    std::vector<std::tuple<int, int>> group;
    std::queue<std::tuple<int, int>> queue;
    std::vector<std::vector<bool>> visited(theta_steps, std::vector<bool>(rho_steps, false));
    ...
    while (!queue.empty()) {
        ...
        for (int dt = -spread; dt <= spread; ++dt) {
            for (int dr = -spread; dr <= spread; ++dr) {
                ...
                int nr = r + dr;
                if (nr >= 0 && nr < rho_steps && !visited[nt][nr] && matrix[nt][nr] > threshold) {
                    visited[nt][nr] = true;
                    queue.push({nt, nr});
                }
            }
...
// 在图像上绘制直线
BmpImage::BmpImage draw_lines(const std::vector<std::tuple<double, double>>& lines, BmpImage::BmpImage& image, BmpImage::BmpPixel color = {255, 0, 0, 255}) {
    ...
    for (const auto& line : lines) {
        ...
        if (std::abs(std::sin(theta)) > 1e-6) {
            ...
            if (y1 >= 0 && y1 < height) points.emplace_back(x1, y1);

            int x2 = width - 1;
            int y2 = static_cast<int>((rho - x2 * std::cos(theta)) / std::sin(theta));
            if (y2 >= 0 && y2 < height) points.emplace_back(x2, y2);
        }
        ...
        if (std::abs(std::cos(theta)) > 1e-6) {
            ...
            if (x1 >= 0 && x1 < width) points.emplace_back(x1, y1);

            int y2 = height - 1;
            int x2 = static_cast<int>((rho - y2 * std::sin(theta)) / std::cos(theta));
            if (x2 >= 0 && x2 < width) points.emplace_back(x2, y2);
        }

        // 如果找到两个端点，则绘制线段
        if (points.size() == 2) {
            Plot::draw_line(image, points[0].first, points[0].second, points[1].first, points[1].second, color);
        }
    }
    return image;
}
...
Point intersect(std::tuple<double, double> line1, std::tuple<double, double> line2) {
...
std::vector<Point> all_intersects(std::vector<std::tuple<double, double>> lines, double parallel_tolerance = 1.0) {
    ...
    for(int i = 0; i < lines.size(); i++) {
        for(int j = i + 1; j < lines.size(); j++) {
            ...
            if (std::abs(theta1 - theta2) < parallel_tolerance || std::abs(theta1 - theta2 + M_PI) < parallel_tolerance || std::abs(theta1 - theta2 - M_PI) < parallel_tolerance) {
                continue;
            }
            Point intersect_point = intersect(lines[i], lines[j]);
            if(std::get<0>(intersect_point) < 0 || std::get<1>(intersect_point) < 0) {
                continue;
            }
