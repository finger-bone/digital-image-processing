...
void cutoff_freq(ComplexMatrix& matrix, double cutoff, bool remove_high = false, double value = 0) {
...
std::tuple<RealMatrix, RealMatrix> polar_transform(const ComplexMatrix& matrix) {
    int n = matrix.size();
    int m = matrix[0].size();
