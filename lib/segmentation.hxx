...
namespace Segmentation
{
  ...
  namespace SegmentationByGrowth
  {
    ...
    std::set<Point> grow_region(
        BmpImage::BmpImage &img_src,
        const std::set<Point> &seeds,
        std::function<bool(
            Point,
            const BmpImage::BmpImage &,
            const std::set<Point> &)>
            validate,
      ...
      while (last_round_any_points_grown)
      {
        ...
        for (const auto &p : region)
        {
          ...
          for (size_t i = 0; i < (eight_direction ? 8 : 4); ++i)
          {
            ...
            if (nx >= 0 && nx < img.image.size.width &&
                ny >= 0 && ny < img.image.size.height)
            {
        ...
        std::set_difference(
            next_try_points.begin(), next_try_points.end(),
  ...
  namespace SegmentationByQuadTree
  {
    ...
    using HomogeneousFunction = std::function<bool(
        const BmpImage::BmpImage &,
        const std::vector<Box> &)>;

    std::shared_ptr<QuadTreeNode> build_quad_tree(const BmpImage::BmpImage &img_src, HomogeneousFunction homogenous_function, Box box)
    {
      ...
      else
      {
        ...
        node->children[0] = build_quad_tree(img_src, homogenous_function, {box.l, mid_x, box.t, mid_y});
        node->children[1] = build_quad_tree(img_src, homogenous_function, {mid_x, box.r, box.t, mid_y});
        node->children[2] = build_quad_tree(img_src, homogenous_function, {box.l, mid_x, mid_y, box.b});
        node->children[3] = build_quad_tree(img_src, homogenous_function, {mid_x, box.r, mid_y, box.b});
        return node;
      }
    }
